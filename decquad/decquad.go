...
var DECCOMBMSD = [64]uint32{0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3, 4, 5, 6, 7,
...
if (c!=cstart) {
  ...
  UBFROMUI(c, UBTOUI(u+3-u3)|CHARMASK)
  c+=u3
}
 }

func DecFloatToString(df DecQuad) (res string, err error) {
	...
	exp = int32(DECCOMBEXP[comb]) // ..
	if exp < DECFLOAT_MinSp {     // finite
		// complete exponent; top two bits are in place
		exp += int32((df[3]&0x03ffffff)>>(32-6-DECECONL)) - 6176 // .. + continuation and unbias
	} else {
		if exp == DECFLOAT_Inf {
			buf.WriteString("Inf")
